...
impl Repl {
    fn new(ast_start: bool) -> Repl {
        ...
        Repl {
            ...
            rl: DefaultEditor::new().expect("could not use as a terminal"), // TODO manage this so we just accept stdin instead
        }
    }
    ...
    fn ast_equality(&mut self, lhs: MathExpr, rhs: MathExpr) -> String {
        if let MathExpr::Term(Term::Multiply(var, Factor::Parenthesis(possible_args))) = lhs {
            ...
            ) = (&*var, &*possible_args)
            {
                ...
                // FIXME: this is not perfect but i think we might need to live with it otherwise we just dont know what it is
                self.approximator
                    .context_mut()
                    .functions
